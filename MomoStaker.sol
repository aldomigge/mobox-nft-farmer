...
interface IMoMoToken {
    ...
    function levelUp(
        uint256 tokenId_,
        uint256[] memory protosV1V2V3_,
        uint256[] memory amountsV1V2V3_,
        uint256[] memory tokensV4V5_
    ) external;
...
contract MoMoStaker is Ownable, ReentrancyGuard {
    ...
    function setRewardMgr(address addr_) external onlyOwner {
        require(
            address(_moboxToken) != address(0) && addr_ != rewardMgr,
            "invalid param"
        );
    ...
    function getTokens()
        external
        view
        returns (address momoToken, address momomToken, address moboxToken)
    {
    ...
    function _earned(
        address user_,
        uint256 rewardPerHashrate_
    ) internal view returns (uint256) {
        UserRewardInfo memory info = _userRewardInfoMap[user_];
        uint256 userHashRate = uint256(info.userHashrateFixed)
            .mul(uint256(info.userHashratePercent) + 10000)
            .div(10000);
        return
            userHashRate
                .mul(
                    rewardPerHashrate_.sub(uint256(info.userRewardPerTokenPaid))
                )
                .add(uint256(info.userReward));
    }
    ...
    function devTeamEarned() external view returns (uint256) {
        if (lastUpdateTimeForDev >= periodFinish) {
            ...
            uint256 lastUpdateDev = Math.max(
                rewardStartTime,
                lastUpdateTimeForDev
            );
            ...
            if (currentUpdateTime <= lastUpdateDev) {
                ...
                return
                    rewardsReleasedForDev.add(
                        currentUpdateTime
                            .sub(lastUpdateDev)
                            .mul(rewardRate)
                            .mul(devTeamRate)
                            .div(10000 - devTeamRate)
                    );
            }
        }
    }
    ...
    function _updateReward(address user_) internal {
        ...
        if (user_ != address(0)) {
            ...
            info.userReward = SafeMathExt.safe128(
                _earned(user_, rewardPerTokenStored)
            );
    ...
    function _updateDevTeamReward() internal {
        if (
            ...
            lastUpdateTimeForDev < periodFinish
        ) {
            ...
            rewardsReleasedForDev = rewardsReleasedForDev.add(
                minTime.sub(lastUpdateDev).mul(rewardRate).mul(devTeamRate).div(
                    10000 - devTeamRate
                )
            );
    ...
    function initReward(
        ...
        rewardRate = rewardPerDay_.mul(10000 - devTeamRate).div(10000).div(
            86400
        );
    ...
    function changeRewardRate(uint256 rewardPerDay_) external {
        require(msg.sender == rewardMgr, "invalid caller");
        require(
            block.timestamp > rewardStartTime && block.timestamp < periodFinish,
            "invaild call"
        );
    ...
    function getDevTeamReward() external {
        require(
            msg.sender == owner() || msg.sender == devTeam,
            "invalid caller"
        );
        _updateDevTeamReward();
    ...
    function _checkCrew(
        ...
        for (uint256 i = 1; i <= 4; ++i) {
            if (
                i != prototype_ / 10000 &&
                _stakingMomoAmountMap[
                    addrPrefix_ + 10000 * i + (prototype_ % 10000)
                ] ==
                0
            ) {
                return false;
    ...
    function _stakeNft(
        ...
        if (ids_.length > 0) {
            if (doTransfer_) {
                _momomToken.safeBatchTransferFrom(
                    user_,
                    address(this),
                    ids_,
                    amounts_,
            ...
            for (i = 0; i < ids_.length; ++i) {
                if (ids_[i] > 0 && amounts_[i] > 0) {
                    require(
                        ids_[i] > 10000 &&
                            ids_[i] < 40000 &&
                            amounts_[i] < 0x08000000,
                        "invalid 1155"
                    );
                    hashrateFixed = hashrateFixed.add(
                        (ids_[i] / 10000).mul(amounts_[i])
                    );

                    prototype = uint256(addrPrefix + ids_[i]);
                    oldAmount = _stakingMomoAmountMap[prototype];
                    _stakingMomoAmountMap[prototype] = oldAmount.add(
                        amounts_[i]
                    );
                    if (oldAmount == 0) {
        ...
        if (tokenIds_.length > 0) {
            ...
            for (i = 0; i < tokenIds_.length; ++i) {
                require(
                    tokenIds_[i] > 0 && tokenIds_[i] < 0x0100000000,
                    "invalid momo"
                );
                if (doTransfer_) {
                    _momoToken.transferFrom(user_, address(this), tokenIds_[i]);
                }
                (prototype, hashrate) = _momoToken.getMomoSimpleByTokenId(
                    tokenIds_[i]
                );
                require(prototype > 40000 && prototype < 65536, "invalid momo");
                hashrateFixed = hashrateFixed.add(hashrate);

                _momoIdToOwnerAndIndex[tokenIds_[i]] = NftOwner(
                    user_,
                    SafeMathExt.safe64(momos.length)
                );
                momos.push(tokenIds_[i]);

                if (prototype / 10000 == 4) {
                    counter.amountV4 = SafeMathExt.add64(
                        counter.amountV4,
                        uint64(1)
                    );

                    oldAmount = _stakingMomoAmountMap[addrPrefix + prototype];
                    _stakingMomoAmountMap[addrPrefix + prototype] = oldAmount
                        .add(1);
                    if (oldAmount == 0) {
    ...
    function _removeNft(
        ...
        if (ids_.length > 0) {
            for (i = 0; i < ids_.length; ++i) {
                ...
                _stakingMomoAmountMap[prototype] = shareParams[1].sub(
                    amounts_[i]
                );
        ...
        if (tokenIds_.length > 0) {
            ...
            for (i = 0; i < tokenIds_.length; ++i) {
                ...
                (prototype, shareParams[1]) = _momoToken.getMomoSimpleByTokenId(
                    tokenIds_[i]
                );
                ...
                if (uint256(nfto.index) != momos.length.sub(1)) {
                    ...
                    _momoIdToOwnerAndIndex[momos[uint256(nfto.index)]]
                        .index = nfto.index;
                }
                momos.pop();
                // realRemove
                ...
                if (prototype / 10000 == 4) {
                    counter.amountV4 = SafeMathExt.sub64(
                        counter.amountV4,
                        uint64(1)
                    );

                    shareParams[1] = _stakingMomoAmountMap[
                        shareParams[0] + prototype
                    ];
                    _stakingMomoAmountMap[
                        shareParams[0] + prototype
                    ] = shareParams[1].sub(1);
        ...
        if (changeType_ > 0) {
            (shareParams[2], shareParams[3]) = _calcHashrate(
                user_,
                0,
                hashrateFixed
            );
    ...
    function _calcHashrate(
        address user_,
        uint256 hashrateFixedAdd_,
        uint256 hashrateFixedSub_
    ) internal returns (uint256 oldHashRate, uint256 newHashRate) {
        UserRewardInfo storage info = _userRewardInfoMap[user_];
        oldHashRate = _userHashrate(
            uint256(info.userHashrateFixed),
            uint256(info.userHashratePercent)
        );
        ...
        if (hashrateFixedSub_ > 0) {
            info.userHashrateFixed = SafeMathExt.sub128(
                ...
                SafeMathExt.safe128(hashrateFixedSub_)
            );
        }
    ...
    function levelUp(
        ...
        require(
            msg.sender == address(_momoIdToOwnerAndIndex[tokenId_].addr),
            "not owner"
        );
        _updateReward(msg.sender);

        // doTransfer = false, realRemove = true
        uint256 hashrateFixedSub = _removeNft(
            msg.sender,
            protosV1V2V3_,
            amountsV1V2V3_,
            tokensV4V5_,
        ...
        uint256 oldHashRate;
        uint256 newHashRate;
        (oldHashRate, newHashRate) = _calcHashrate(
            msg.sender,
            hashrateLevelUp,
            hashrateFixedSub
        );
        emit LevelUpBurnStake(
            msg.sender,
            protosV1V2V3_,
            amountsV1V2V3_,
            tokensV4V5_
    ...
    function _isRenting(uint256 tokenId_) internal view returns (bool) {
        return
            momoRenter == address(0)
                ? false
                : IMoMoRenter(momoRenter).isRenting(tokenId_);
    }
}
