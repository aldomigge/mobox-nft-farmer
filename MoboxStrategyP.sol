...
contract MoboxStrategyP is Pausable, ReentrancyGuard {
    ...
    // Pancake MasterChef
    address public constant pancakeFarmer =
        0x73feaa1eE314F8c655E354234017bE2193C9E24E;
    // Pancake Swap rounter
    address public constant pancakeRouterV2 =
    ...
    function init(
        ...
        require(
            pancakePid == 0 && moboxFarm == address(0),
            "may only be init once"
        );
        require(
            pId_ > 0 && moboxFarm_ != address(0) && buyBackPool_ != address(0),
            "invalid param"
        );
        require(
    ...
    function getTotal()
        external
        view
        returns (uint256 wantTotal_, uint256 sharesTotal_)
    {
    ...
    function withdraw(
        ...
        if (lpBalance < wantAmount) {
            IPancakeMasterChef(pancakeFarmer).withdraw(
                pancakePid,
                wantAmount.sub(lpBalance)
            );
    ...
    function harvest() external whenNotPaused nonReentrant {
        ...
        uint256 lpAmountBeforeHarvest = IERC20(wantToken).balanceOf(
            address(this)
        );
        if (cakeAmount > 0) {
            ...
            if (cakeAmount > 0) {
                ...
                if (cake != tokenA) {
                    IPancakeSwapRouter(pancakeRouterV2)
        ...
        uint256 lpAmountAfterHarvest = IERC20(wantToken).balanceOf(
            address(this)
        );
        // Deposit the unstaked LP and the reinvested LP into pancake together
        if (lpAmountAfterHarvest > lpAmountBeforeHarvest) {
            wantTotal = wantTotal.add(
                lpAmountAfterHarvest.sub(lpAmountBeforeHarvest)
            );
            IPancakeMasterChef(pancakeFarmer).deposit(
                pancakePid,
                lpAmountAfterHarvest
            );
        }
    }
    ...
    function rebalance() external onlyStrategist {
        uint256 amount;
        (amount, ) = IPancakeMasterChef(pancakeFarmer).userInfo(
            pancakePid,
            address(this)
        );
    ...
    function dustToEarnToken(address dustToken_) external onlyStrategist {
        ...
        if (dustAmount > 0) {
            IERC20(dustToken_).safeIncreaseAllowance(
                pancakeRouterV2,
                dustAmount
            );
            IPancakeSwapRouter(pancakeRouterV2)
